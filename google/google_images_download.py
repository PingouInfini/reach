...
class googleimagesdownload:
    ...
    def download_page(self,url):
        try:
            ...
            logging.debug("Could not open URL. Please check your internet connection and/or ssl settings")
    ...
    def download_extended_page(self,url,chromedriver):
        ...
        try:
            ...
            logging.debug("Looks like we cannot locate the path the 'chromedriver' (use the '--chromedriver' "
        ...
        browser.get(url)
        time.sleep(1)
        logging.debug("Getting you a lot of images. This may take a few moments...")
    ...
    def get_next_tab(self,s):
        ...
        if start_line == -1:  # If no links are found then give an error!
            ...
            #logging.debug(url_item,url_item_name)
    ...
    def single_image(self,image_url):
        ...
        logging.debug("completed ====> " + image_name)
        return
    ...
    def keywords_from_file(self,file_name):
        ...
        with codecs.open(file_name, 'r', encoding='utf-8-sig') as f:
            if '.csv' in file_name:
                ...
                logging.debug("Invalid file type: Valid file types are either .txt or .csv \n"
                      "exiting...")
                sys.exit()
    ...
    # Download Images
    def download_image(self,image_url,image_format,main_directory,dir_name, image_name, count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download):
        if print_urls or no_download:
            logging.debug("Image URL: " + image_url)
        if no_download:
        ...
        try:
            ...
            try:
                ...
                # get the extension (check only the last characters to avoid undesirable "."
                ...
                if print_size:
                    logging.debug("Image Size: " + str(self.file_size(path)))
    ...
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments):
        ...
        while count < limit+1:
            ...
            if object == "no_links":
                ...
                #download the images
                try:
                    download_status,download_message,return_image_name = self.download_image(object['image_link'],object['image_format'],main_directory,dir_name,object['image_description'],count,arguments['print_urls'],arguments['socket_timeout'],arguments['prefix'],arguments['print_size'],arguments['no_numbering'],arguments['no_download'])
                    logging.debug(download_message)
                except:
                    download_status, download_message_thumbnail,return_image_name = self.download_image(object['image_thumbnail_url'],object['image_format'],main_directory,dir_name,object['image_description'],count,arguments['print_urls'],arguments['socket_timeout'],arguments['prefix'],arguments['print_size'],arguments['no_numbering'],arguments['no_download'])
                    logging.debug(download_message_thumbnail)
            ...
            for sky in suffix_keywords:     # 1.for every suffix keywords
                ...
                while i < len(search_keyword):      # 2.for every main keyword
                    ...
                    if arguments['no_download']:
                        logging.debug("Starting to Print Image URLS")
                    else:
                    ...
                    if arguments['extract_metadata']:
                        json_file = open(utils.get_directory_from_property('GoogleImages', 'googleimages.metadatadirectory')
